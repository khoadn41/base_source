...
export function getCurrentDateYYYY_MM_DD(format: string = '/'): string {
  return moment().format(`YYYY${format}MM${format}DDT00:00:00.000Z`);
}

export function compareTime(firstTime: string, secondTime: string): number {
  if (moment(firstTime, 'HH:mm:ss').isBefore(moment(secondTime, 'HH:mm:ss'))) {
    return -1;
  }

  if (moment(firstTime, 'HH:mm:ss').isAfter(moment(secondTime, 'HH:mm:ss'))) {
    return 1;
  }
  return 0;
}

export function converMinuteToTime(totalMinutes: number): string {
